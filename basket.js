...
for(let item of basket){
    ...
    `
    <h4>$<span class="subtotal-value">${Math.round(Number(item.price)*Number(item.count))}</span></h4>
    `;
    // Remove td
    ...
    `;
    tr.append(checkBoxtd,imagetd,prodNametd,prodfPricetd,prodCounttd,prodsPricetd,prodRemovetd,productIdw);
    tbody.append(tr);
    ...
    for(let a= 0;a< basket.length;a++){
        ...
        for(let inp of myInps){ 
            ...
            increaseBtn.addEventListener("click",function(e){
                e.preventDefault();
                if(this.parentElement.parentElement.parentElement.querySelector(".product-id").innerText === basket[a].id){
                    let myInp = inp.querySelector(".count");
                    basket[a].count+=1;
                    myInp.innerText = basket[a].count;
                    this.parentElement.parentElement.nextElementSibling.querySelector(".subtotal-value").innerText = Math.round(Number(basket[a].price * basket[a].count));
                    totalPrice();
                    localStorage.setItem("basket",JSON.stringify(basket));
                }
            })
            decreaseBtn.addEventListener("click",function(e){
                e.preventDefault();
                if(basket[a].count != 0 && this.parentElement.parentElement.parentElement.querySelector(".product-id").innerText === basket[a].id){
                    let myInp = inp.querySelector(".count");
                    basket[a].count-=1;
                    myInp.innerText = basket[a].count;
                    this.parentElement.parentElement.nextElementSibling.querySelector(".subtotal-value").innerText = Math.round(Number(basket[a].price * basket[a].count));
